...
(com memória + filtro dinâmico por mês/ano + validação anti-alucinação
 ...
 + (Opcional) fallback de LLM para OpenAI se você quiser ativar)

Requisitos (mínimo para Gemini + Vertex):
    pip install -U langchain langchain-google-vertexai langchain-chroma \
...
(Opcional se quiser fallback com OpenAI):
    pip install -U langchain-openai openai httpx
...
# Carrega .env antes de ler flags/variáveis
load_dotenv()

# --- (Opcional) OpenAI como fallback (desativado por padrão) ---
USE_OPENAI_FALLBACK = bool(os.getenv("USE_OPENAI_FALLBACK", "").strip())
if USE_OPENAI_FALLBACK:
    try:
        from langchain_openai import OpenAIEmbeddings, ChatOpenAI
        # APITimeoutError é opcional; exceções genéricas já cobrem
        from openai import APITimeoutError  # type: ignore
        import httpx  # pode ajudar a capturar timeouts
    except Exception:
        USE_OPENAI_FALLBACK = False  # bibliotecas não instaladas; ignora fallback
...
INDEX_DISPLAY_NAME    = os.getenv("VERTEX_INDEX_NAME", "indices-financeiros-gisa-v1")
ENDPOINT_DISPLAY_NAME = os.getenv("VERTEX_ENDPOINT_NAME", "endpoint-financeiros-gisa-v1")

# Modelos Gemini (use IDs versionados; troque por -002, -pro-002, etc. se necessário)
GEMINI_QA_MODEL    = os.getenv("GEMINI_QA_MODEL", "gemini-2.5-flash")
GEMINI_VALID_MODEL = os.getenv("GEMINI_VALID_MODEL", "gemini-2.5-flash")

# text-embedding-005 => 768 dims
VECTOR_DIMENSIONS = int(os.getenv("VERTEX_VECTOR_DIM", "768"))

# Flag opcional para forçar a criação de um novo índice quando houver
# suspeita de inconsistência entre Matching Engine e docstore (GCS).
FORCE_NEW_INDEX = os.getenv("VERTEX_FORCE_NEW_INDEX", "").strip() == "1"
...
# (se você habilitar OpenAI, estes valores serão usados)
...
# LLMs primários (Gemini) — fixe project/location e use IDs versionados
LLM_QA_VERTEX    = ChatVertexAI(model=GEMINI_QA_MODEL,    temperature=0.5, project=PROJECT_ID, location=REGION)
LLM_VALID_VERTEX = ChatVertexAI(model=GEMINI_VALID_MODEL, temperature=0.0, project=PROJECT_ID, location=REGION)
...
if USE_OPENAI_FALLBACK:
    ...
    LLM_VALID_OPENAI = ChatOpenAI(model="gpt-4o-mini", temperature=0.0,
                                  timeout=OPENAI_CHAT_TIMEOUT, max_retries=OPENAI_MAX_RETRIES)

VALIDATOR_VERTEX = PROMPT_VALID | LLM_VALID_VERTEX
VALIDATOR_OPENAI = (PROMPT_VALID | LLM_VALID_OPENAI) if USE_OPENAI_FALLBACK else None
...
def run_llm_analytics(question: str, dados: str) -> str:
    """
    Usa Gemini por padrão. Se USE_OPENAI_FALLBACK=True e Gemini falhar,
    tenta OpenAI. Caso contrário, retorna erro.
    """
    try:
        ...
        if USE_OPENAI_FALLBACK:
            try:
                ...
                return f"Falha na análise (Gemini e OpenAI indisponíveis): {e}"
        return f"Falha na análise (Gemini indisponível): {e}"
...
def ensure_vertex_index_and_endpoint(
    ...
    if FORCE_NEW_INDEX:
        ...
        index_display_name = f"{index_display_name}-{ts}"
    ...
    if idx_id not in deployed_ids:
        ...
        # Algumas versões aceitam sync=..., outras não. Tentamos sem sync
        # e depois fazemos polling caso não haja .result().
        try:
            op = endpoint.deploy_index(index=index, deployed_index_id=dep_id, sync=False)  # type: ignore
        except TypeError:
        ...
        if hasattr(op, "result"):
            # Versões em que retorna LRO
            op.result(timeout=3600)
            print("   Deploy concluído.")
        else:
            # Polling universal: espera o índice aparecer como 'deployed'
            deadline = time.time() + 3600  # 60 min
            while time.time() < deadline:
                endpoint = aiplatform.MatchingEngineIndexEndpoint(endpoint.resource_name)
                deployed_ids = [d.index.split("/")[-1] for d in endpoint.gca_resource.deployed_indexes]
                if idx_id in deployed_ids:
                    print("   Deploy concluído.")
                    break
                time.sleep(10)
            else:
                raise TimeoutError("Deploy Index não concluiu no prazo (60 min).")
...
def choose_loader(path: str) -> str:
    ...
    return "line" if {"mes_ano", "valor", "tipo", "empresa_id"}.issubset(cols) else "cross"  # CORRIGIR? ver abaixo
    # OBS: Se sua base "linha por valor" usa 'mes_ano', troque 'mes_ao' por 'mes_ano' ↑

# ========= 8. Parse indicador / mês / ano ========= #
...
def parse_meses_ano(pergunta: str):
    ...
    Entende padrões como: "maio e junho de 2025", "de maio para junho de 2025",
...
def validar_resposta(pergunta: str, provisoria: str, fontes_docs: List[Document]) -> str:
    ...
    # 1) Tenta validar com Gemini
    try:
        feedback = VALIDATOR_VERTEX.invoke(
    ...
    # 2) (Opcional) fallback de validação com OpenAI
    if USE_OPENAI_FALLBACK and VALIDATOR_OPENAI is not None:
        try:
            feedback = VALIDATOR_OPENAI.invoke(
                {"question": pergunta, "draft_answer": provisoria, "sources": fontes_txt, "hoje": hoje, "n_sources": str(n_sources)}
            ).content.strip()
            return provisoria if feedback == "PASS" else feedback
        except Exception:
    ...
    # 3) Se ambos indisponíveis, retorna a provisória avisando
...
def invoke_with_fallback(vector_primary, file_path, empresa_id, memory, question,
    ...
    """
    Ordem de fallback:
    1) Retriever atual + LLM Gemini (primário)
    2) (Opcional) Mesmo retriever + LLM OpenAI
    3) Fallback local (Chroma+HF) + LLM Gemini (ou OpenAI, se Gemini indisponível)
    ...
    try:
        ...
        # Caso típico: índice retorna IDs antigos que não existem no docstore atual
        if "not found in the storage" in str(e).lower():
            print("⚠️  IDs retornados pelo Vertex não têm payload no docstore atual. Caindo para fallback local (Chroma + HF).")
        else:
            raise
    except Exception:
        pass

    # 2) (Opcional) tenta com OpenAI (se habilitado)
    if USE_OPENAI_FALLBACK:
        try:
            qa_openai = _build_chain(retriever, memory, LLM_QA_OPENAI)
            return _invoke_chain(qa_openai, memory)
        except Exception:
            pass

    # 3) fallback local (Chroma + HF) + Gemini (ou OpenAI se Gemini indisponível)
    try:
        ...
        try:
            ...
            if USE_OPENAI_FALLBACK:
                qa2 = make_chain_no_memory(retriever2, llm=LLM_QA_OPENAI)
                res2 = qa2.invoke({"question": question, "chat_history": []})
            else:
                raise e_inner
        if "answer" in res2 and isinstance(res2["answer"], str):
            res2["answer"] += "\n\n(Nota: usei índice local por indisponibilidade temporária do Vertex/LLM.)"
        return res2
    except Exception as e:
...
# ========= 11. Vetor + memória (construtor principal) ========= #
def build_vector_and_memory(file_path: str, empresa_id: int, bucket: str, index_id: str, endpoint_id: str):
    ...
    # Memória resumida usando GEMINI (primário)
...
if __name__ == "__main__":
    ...
    # Para limpar o fallback local, descomente:
